...
class BabyMonitorApp:
    def __init__(self, root):
        ...
        self.param_danger_dist = tk.DoubleVar(value=100.0)  # 아기 주변 위험요소 거리 기본값 1m로 설정
    ...
    def setup_ui(self):
        ...
        tk.Label(settings_frame, text="Pushover User Key:").grid(row=0, column=0, padx=5, pady=5)
        tk.Entry(settings_frame, textvariable=self.pushover_user_key).grid(row=0, column=1, padx=5, pady=5)

        tk.Label(settings_frame, text="Pushover API Key:").grid(row=1, column=0, padx=5, pady=5)
        tk.Entry(settings_frame, textvariable=self.pushover_api_key).grid(row=1, column=1, padx=5, pady=5)

        tk.Label(settings_frame, text="No Head Detection Alert (seconds):").grid(row=2, column=0, padx=5, pady=5)
        tk.Entry(settings_frame, textvariable=self.param_nohead_sec).grid(row=2, column=1, padx=5, pady=5)

        tk.Label(settings_frame, text="아기가 배로 누운지 몇 초 지나서 알람 발송할지:").grid(row=3, column=0, padx=5, pady=5)
        tk.Entry(settings_frame, textvariable=self.param_prone_sec).grid(row=3, column=1, padx=5, pady=5)

        tk.Label(settings_frame, text="아기 주변에 위험요소가 얼마나 가까이 있을 때 알림 발송할지:").grid(row=4, column=0, padx=5, pady=5)
        tk.Entry(settings_frame, textvariable=self.param_danger_dist).grid(row=4, column=1, padx=5, pady=5)

        tk.Checkbutton(settings_frame, text="Pushover 알림 전송", variable=self.enable_pushover).grid(row=5, columnspan=2, pady=10)
        tk.Button(settings_frame, text="Pushover 알림 테스트", command=self.test_pushover).grid(row=6, columnspan=2, pady=10)

        # 배속 슬라이더
        tk.Label(settings_frame, text="배속:").grid(row=7, column=0, padx=5, pady=5)
        tk.Scale(settings_frame, from_=0.5, to=2.0, resolution=0.1, orient=tk.HORIZONTAL, variable=self.playback_speed).grid(row=7, column=1, padx=5, pady=5)

        # 버튼 프레임
        button_frame = tk.Frame(self.root)
        button_frame.pack(pady=10)

        tk.Button(button_frame, text="영상 파일 열기", command=self.open_file).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="동영상 파일 열기", command=self.open_video_file).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="실시간 카메라 시작", command=self.start_camera).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="실시간 카메라 종료", command=self.stop_camera).pack(side=tk.LEFT, padx=5)
